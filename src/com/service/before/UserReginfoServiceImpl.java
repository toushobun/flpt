...
public class UserReginfoServiceImpl implements UserReginfoService {
	...
	public String userAddReginfo(Reginfo reginfo, Model model) {
		...
		Testinfo__room testinfo__room = userReginfoDao
				.userSelectATestinfo__roomByTestinfo__room_id(reginfo.getTestinfo__room_id());
		// 准考证的组成：1~2位是年份，3~4位是考试编号后两位，5~6位是考场编号后两位，
		// 7~8位是考生id后两位（X识别为0），9~12位是候补位，如果重复则整体+1直到不重复为止
		String ticketnum_default = testinfo__room.getTest_time().substring(2, 4) + testinfo__room.getTestinfo_id() % 100
				+ testinfo__room.getRoom_id() % 100 + reginfo.getUser_id() % 100;
		...
		if (userReginfoDao.userSelectATestinfo__roomByTestinfo__room_id(reginfo.getTestinfo__room_id())
				.getRquota() <= 0) {
	...
	public String userCancelReginfo(Integer reginfo_id, Model model) {
		...
		if (adminTestinfoDao.updateTestinfo__room(testinfo__room) > 0
				&& userReginfoDao.userDeleteReginfoByReginfo_id(reginfo_id) > 0) {
	...
	public String userToChangeRoom(Reginfo reginfo, Model model) {
		...
		Testinfo__room oldTestinfo__room = userReginfoDao
				.userSelectATestinfo__roomByTestinfo__room_id(reginfo.getTestinfo__room_id());
		List<Testinfo__room> testinfo__roomList = userReginfoDao
				.userSelectTestinfo__room(oldTestinfo__room.getTestinfo_id());
		testinfo__roomList = MyUtil.pushTestinfo__roomToBuser(testinfo__roomList);
	...
	public String userChangeRoom(Reginfo reginfo, Model model) {
		...
		// 旧考场的名额+1
		Testinfo__room oldTestinfo__room = userReginfoDao
				.userSelectATestinfo__roomByTestinfo__room_id(reginfo.getOldTestinfo__room_id());
		...
		Testinfo__room testinfo__room = userReginfoDao
				.userSelectATestinfo__roomByTestinfo__room_id(reginfo.getTestinfo__room_id());
		testinfo__room.setRquota(testinfo__room.getRquota() - 1);
		if (userReginfoDao.userUpdateReginfo(reginfo) > 0 && adminTestinfoDao.updateTestinfo__room(testinfo__room) > 0
				&& adminTestinfoDao.updateTestinfo__room(oldTestinfo__room) > 0) {
